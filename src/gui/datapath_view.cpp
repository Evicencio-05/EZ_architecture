...
static void centerText(sf::Text& label) {
  ...
  label.setOrigin({text_bounds.position.x + (textBounds.size.x / 2.0F),
                   text_bounds.position.y + (textBounds.size.y / 2.0F)});
}
...
void DatapathView::set_position(float x, float y) {
...
void DatapathView::set_size(float width, float height) {
  m_width = width;
  m_height = height;
  calculateLayout();
}

void DatapathView::draw_scaffolding(sf::RenderWindow& window,
                                    sf::Vector2f main_area_size) {
  int width = static_cast<int>(main_area_size.y);
  int height = static_cast<int>(main_area_size.x);
...
void DatapathView::calculate_layout() {
...
void DatapathView::update_wire_position(float x, float y) {
  for (Wire& wire : m_wires) {
    size_t length = wire.vertices->getVertexCount();
    for (size_t i = 0; i < length; ++i) {
      sf::Vector2f current_position = wire.vertices->operator[](i).position;
      wire.vertices->operator[](i).position = sf::Vector2f(
          current_position.x - m_x + x, current_position.y - m_y + y);
    }
  }
}

void DatapathView::update_wire_label_position(float x, float y) {
  for (size_t i = 0; i < DatapathView::NUMBER_OF_WIRE_LABLES; ++i) {
    sf::Vector2f current_positon = m_wireLabels[i].position;
    m_wireLabels[i].position = {current_positon.x - m_x + x,
                                current_positon.y - m_y + y};
  }
}
...
void DatapathView::update() {
  ...
  word_t raw_instruction = m_cpu.get_current_instruction().get_raw();
  Decoder decoder;
  std::string current_instruction_name =
      ez_arch::Decoder::get_details(raw_instruction).mnemonic;

  for (auto& wire : m_wires) {
    wire.active = true;
  }

  std::vector<size_t> off_wires;
  off_wires.reserve(m_wires.size());
}
...
void DatapathView::draw_component_box(sf::RenderWindow& window,
  ...
  size_t inputs_size = box.inputs.size();
  ...
  size_t outputs_size = box.outputs.size();
...
void DatapathView::draw_ellipse(sf::RenderWindow& window,
  ...
  sf::Vector2f circle_radius = circle->getRadius();
  label.setPosition(sf::Vector2f(circle_pos.x + circle_radius.x,
                                 circle_pos.y + circle_radius.y));
  center_text(label);
  window.draw(label);
}

static void DatapathView::draw_alu(sf::RenderWindow& window,
...
static void DatapathView::draw_mux(sf::RenderWindow& window,
...
static void DatapathView::draw_gate(sf::RenderWindow& window,
...
static void DatapathView::draw_wire(sf::RenderWindow& window, Wire& wire) {
  ...
  size_t length = wire.vertices->getVertexCount();
  for (size_t i = 0; i < length; ++i) {
    wire.vertices->operator[](i).color = line_color;
  }
  ...
  if (!wire.no_arrow && length >= 2) {
    sf::Vector2f back_positon = wire.vertices->operator[](length - 1).position;
    sf::VertexArray arrowhead(sf::PrimitiveType::Triangles, 3);
    arrowhead[0].position =
        sf::Vector2f(back_positon.x - 5.F, back_positon.y - 6.F);
    arrowhead[0].color = line_color;
    arrowhead[1].position = back_positon;
    arrowhead[1].color = line_color;
    arrowhead[2].position =
        sf::Vector2f(back_positon.x - 5.F, back_positon.y + 5.F);
    arrowhead[2].color = line_color;

    window.draw(arrowhead);
  }
}

void DatapathView::setup_wires() {
  for (Wire& wire : m_wires) {
    wire.vertices->clear();
  }
...
void DatapathView::setup_wire_labels() {
  // 4
...
void DatapathView::draw_label(sf::RenderWindow& window, WireLabel& wire_lable) {
  sf::Text label(m_font, wire_lable.label, 12);
  label.setFillColor(sf::Color::Black);
  label.setPosition(wire_lable.position);
  center_text(label);
  window.draw(label);
}
