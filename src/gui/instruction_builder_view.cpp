...
void InstructionBuilderView::setPosition(float x, float y) { m_x = x; m_y = y; }
void InstructionBuilderView::setSize(float w, float h) { m_w = w; m_h = h; }

void InstructionBuilderView::handleMouseMove(float, float) {}
...
void InstructionBuilderView::handleMousePress(float x, float y) {
  ...
  float ddw = 180.f, ddh = 22.f;
  float opx = m_x + 70, opy = m_y + 48;
  float rsx = m_x + 70, rsy = m_y + 78;
  float rtx = m_x + 70, rty = m_y + 108;
  float rdx = m_x + 70, rdy = m_y + 138;
  const int vis = 8;
  ...
  if (within(x, y, m_x + 70, m_y + 165, 120, 24)) { m_immActive = true; closeAll(); return; }
...
void InstructionBuilderView::handleMouseWheel(float x, float y, float delta) {
  ...
  auto clampScroll = [&](int& scroll, int size){
    ...
    if (scroll > size - vis) scroll = std::max(0, size - vis);
  };
  int dir = (delta > 0) ? -1 : (delta < 0 ? 1 : 0);
  if (!dir) return;
  if (m_opOpen && within(x, y, opx, opy + ddh, ddw, ddh * vis)) { m_opScroll += dir; clampScroll(m_opScroll, static_cast<int>(kOps.size())); }
  if (m_rsOpen && within(x, y, rsx, rsy + ddh, ddw, ddh * vis)) { m_rsScroll += dir; clampScroll(m_rsScroll, 32); }
  if (m_rtOpen && within(x, y, rtx, rty + ddh, ddw, ddh * vis)) { m_rtScroll += dir; clampScroll(m_rtScroll, 32); }
  if (m_rdOpen && within(x, y, rdx, rdy + ddh, ddw, ddh * vis)) { m_rdScroll += dir; clampScroll(m_rdScroll, 32); }
}

static void draw_dropdown(sf::RenderWindow& window, sf::Font& font,
...
void InstructionBuilderView::draw(sf::RenderWindow& window) {
  ...
  // Dropdowns
  float ddw = 100.f, ddh = 22.f;
  float opx = m_x + 70, opy = m_y + 50;
  float rsx = m_x + 70, rsy = m_y + 80;
  float rtx = m_x + 70, rty = m_y + 110;
  float rdx = m_x + 70, rdy = m_y + 140;
  const int vis = 8;
  ...
  regs.reserve(32);
  for (int i = 0; i < 32; ++i) {
    regs.emplace_back(std::string(REGISTER_NAMES[i]));
  }
  draw_dropdown(window, m_font, opx, opy, ddw, ddh,
      m_opIndex,
      std::vector<std::string>(kOps.begin(), kOps.end()), m_opOpen, m_opScroll, vis);
  if (!m_opOpen) 
    draw_dropdown(window, m_font, rsx, rsy, ddw, ddh, m_rs, regs, m_rsOpen, m_rsScroll, vis);
  if (!m_opOpen && !m_rsOpen) 
    draw_dropdown(window, m_font, rtx, rty, ddw, ddh, m_rt, regs, m_rtOpen, m_rtScroll, vis);
  if (!m_opOpen && !m_rsOpen && !m_rtOpen)
    draw_dropdown(window, m_font, rdx, rdy, ddw, ddh, m_rd, regs, m_rdOpen, m_rdScroll, vis);

  // Live preview and inline validation
  const std::string preview = currentAssembly();
  try {
    (void)Decoder::assemble(preview);
    m_lastError.clear();
  } catch (const std::exception& e) {
    m_lastError = e.what();
  } catch (...) {
  ...
  drawLabel(window, std::string("Preview: ") + preview, m_x + 250.f, m_y + 100);
...
void InstructionBuilderView::drawPanel(sf::RenderWindow& window) {
...
void InstructionBuilderView::drawLabel(sf::RenderWindow& window, const std::string& text, float x, float y, unsigned size) {
  sf::Text t(m_font);
  t.setString(text);
...
void InstructionBuilderView::drawTextBox(sf::RenderWindow& window, const std::string& value, float x, float y, float w, float h, bool active) {
  sf::RectangleShape box({w, h});
  box.setPosition({x, y});
  box.setFillColor(sf::Color::White);
...
void InstructionBuilderView::drawButton(sf::RenderWindow& window, const std::string& text, float x, float y, float w, float h, bool pressed) {
  sf::RectangleShape btn({w, h});
  btn.setPosition({x, y});
  btn.setFillColor(pressed ? BUTTON_PRESSED_COLOR : BUTTON_NORMAL_COLOR);
  ...
  t.setString(text);
  t.setCharacterSize(14);
  t.setFillColor(BUTTON_TEXT_COLOR);
...
std::string InstructionBuilderView::currentAssembly() const {
  ...
  const std::string op = kOps[m_opIndex];
  if (op == "add" || op == "sub" || op == "and" || op == "or" || op == "slt") {
    ss << op << " " << REGISTER_NAMES[m_rd] << ", " << REGISTER_NAMES[m_rs] << ", " << REGISTER_NAMES[m_rt];
  } else if (op == "addi" || op == "andi" || op == "ori") {
    ss << op << " " << REGISTER_NAMES[m_rt] << ", " << REGISTER_NAMES[m_rs] << ", " << (m_imm.empty() ? "0" : m_imm);
  } else if (op == "lw" || op == "sw") {
    ss << op << " " << REGISTER_NAMES[m_rt] << ", " << (m_imm.empty() ? "0" : m_imm) << "(" << REGISTER_NAMES[m_rs] << ")";
  } else if (op == "beq" || op == "bne") {
    ss << op << " " << REGISTER_NAMES[m_rs] << ", " << REGISTER_NAMES[m_rt] << ", " << (m_imm.empty() ? "0" : m_imm);
  }
  return ss.str();
}
...
void InstructionBuilderView::handleKeyPressed(int keyCode) {
  ...
  auto clampIndex = [](int& idx, int max){ if (idx < 0) idx = 0; if (idx >= max) idx = max - 1; };
  const int vis = 8;
  ...
  if (anyOpen) {
    if (keyCode == 73 /*Up*/) {
      if (m_opOpen) { --m_opIndex; clampIndex(m_opIndex, static_cast<int>(kOps.size())); ensureVisible(m_opIndex, m_opScroll, static_cast<int>(kOps.size())); }
      else if (m_rsOpen) { --m_rs; clampIndex(m_rs, 32); ensureVisible(m_rs, m_rsScroll, 32); }
      else if (m_rtOpen) { --m_rt; clampIndex(m_rt, 32); ensureVisible(m_rt, m_rtScroll, 32); }
      else if (m_rdOpen) { --m_rd; clampIndex(m_rd, 32); ensureVisible(m_rd, m_rdScroll, 32); }
      return;
    }
    if (keyCode == 74 /*Down*/) {
      if (m_opOpen) { ++m_opIndex; clampIndex(m_opIndex, static_cast<int>(kOps.size())); ensureVisible(m_opIndex, m_opScroll, static_cast<int>(kOps.size())); }
