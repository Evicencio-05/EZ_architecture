...
static std::string decodeRType(const Instruction& instruction) {
  uint8_t funct = instruction.getFunct();
  uint8_t rs = instruction.getRs();
  uint8_t rt = instruction.getRt();
...
static std::string decodeIType(const Instruction& instruction) {
  ...
  uint8_t rs = instruction.getRs();
  uint8_t rt = instruction.getRt();
  int16_t imm = instruction.getImmediate();
  ...
  case Opcode::kSW:
  case Opcode::kLW:
    ss << mnemonic << " " << kREGISTER_NAMES[rt] << ", " << imm << "("
       << kREGISTER_NAMES[rs] << ")";
    break;

  case Opcode::kBEQ:
  case Opcode::kBNE:
    ss << mnemonic << " " << kREGISTER_NAMES[rs] << ", " << kREGISTER_NAMES[rt]
       << ", " << imm;
    break;

  default:
    ss << mnemonic << " " << kREGISTER_NAMES[rt] << ", " << kREGISTER_NAMES[rs]
       << ", " << imm;
    break;
  }
...
static std::string decodeJType(const Instruction& instruction) {
  uint8_t opcode = instruction.getOpcode();
  uint32_t addr = instruction.getAddress();
...
std::string Decoder::decode(word_t instruction) {
  ...
  case InstructionFormat::R_TYPE:
    return decodeRType(instr);
  case InstructionFormat::I_TYPE:
    return decodeIType(instr);
  case InstructionFormat::J_TYPE:
    return decodeJType(instr);
  }
...
Decoder::InstructionDetails Decoder::getDetails(word_t instruction) {
  ...
  case InstructionFormat::R_TYPE:
    details.fields.push_back("opcode: " + std::to_string(instr.getOpcode()));
    details.fields.push_back("rs: " + std::to_string(instr.getRs()));
    details.fields.push_back("rt: " + std::to_string(instr.getRt()));
    details.fields.push_back("rd: " + std::to_string(instr.getRd()));
    details.fields.push_back("shamt: " + std::to_string(instr.getShamt()));
    details.fields.push_back("funct: 0x" + std::to_string(instr.getFunct()));
  ...
  case InstructionFormat::I_TYPE:
    ...
    details.fields.push_back("rs: " + std::to_string(instr.getRs()));
    details.fields.push_back("rt: " + std::to_string(instr.getRt()));
    details.fields.push_back("immediate: 0x"
  ...
  case InstructionFormat::J_TYPE:
    details.fields.push_back("opcode: " + std::to_string(instr.getOpcode()));
    details.fields.push_back("address: " + std::to_string(instr.getAddress()));
...
static std::vector<std::string> split(const std::string& str,
  ...
  while (start < str.length()) {
    while (start < str.length()
           && delims.find(str[start]) != std::string::npos) {
      start++;
    }
...
static uint8_t parseRegister(const std::string& regStr) {
  ...
  if (reg[0] == '$') {
    ...
    for (size_t i = 0; i < kREGISTER_NAMES.size(); ++i) {
      if (reg == kREGISTER_NAMES[i]) { return static_cast<uint8_t>(i); }
    }
  }
...
static int32_t parseImmediate(const std::string& immStr) {
  ...
  if (imm.empty()) { throw std::invalid_argument("Empty immediate value"); }
...
struct AssembledInstruction {
  ...
  [[nodiscard]] word_t encodeRType() const {
    return (opcode << 26) | (rs << 21) | (rt << 16) | (rd << 11) | (0 << 6)
           | funct;
  }
...
static void validateOperandCount(const ParsedOperands& ops,
                                 size_t expected,
...
static AssembledInstruction assembleRType(const ParsedOperands& ops,
  ...
  if (mnemonic == "add") {
    instruction.funct = Funct::kADD;
  } else if (mnemonic == "sub") {
    instruction.funct = Funct::kSUB;
  } else if (mnemonic == "and") {
    instruction.funct = Funct::kAND;
  } else if (mnemonic == "or") {
    instruction.funct = Funct::kOR;
  } else if (mnemonic == "slt") {
    instruction.funct = Funct::kSLT;
  } else {
    throw std::invalid_argument("Unknown R-type instruction: " + mnemonic);
  }

  return instruction;
}

static AssembledInstruction assembleImmediateArith(
    const ParsedOperands& ops, const std::string& mnemonic) {
  validateOperandCount(ops, 3, mnemonic);
  ...
  if (mnemonic == "addi") {
    instruction.opcode = Opcode::kADDI;
  } else if (mnemonic == "andi") {
    instruction.opcode = Opcode::kANDI;
  } else if (mnemonic == "ori") {
    instruction.opcode = Opcode::kORI;
  } else {
    throw std::invalid_argument("Unknown immediate arithmetic: " + mnemonic);
  }

  return instruction;
}

static std::pair<int16_t, uint8_t> parseMemoryReference(
  ...
  std::string baseStr =
      trim(memRef.substr(parenStart + 1, parenEnd - parenStart - 1));
...
static AssembledInstruction assembleLoadStore(const ParsedOperands& ops,
  ...
  if (mnemonic == "lw") {
    instruction.opcode = Opcode::kLW;
  } else if (mnemonic == "sw") {
    instruction.opcode = Opcode::kSW;
  } else {
    throw std::invalid_argument("Unknown load/store: " + mnemonic);
  }
...
static AssembledInstruction assembleBranch(const ParsedOperands& ops,
  ...
  if (mnemonic == "beq") {
    instruction.opcode = Opcode::kBEQ;
  } else if (mnemonic == "bne") {
    instruction.opcode = Opcode::kBNE;
  } else {
    throw std::invalid_argument("Unknown branch instruction: " + mnemonic);
  }
...
static AssembledInstruction assembleJump(const ParsedOperands& ops,
  ...
  if (mnemonic == "j") {
    ...
    instruction.opcode = Opcode::kJAL;
  } else {
    throw std::invalid_argument("Unknown jump instruction: " + mnemonic);
  }
...
word_t Decoder::assemble(const std::string& assemblyLine) {
  ...
  std::string mnemonic =
      (spacePos != std::string::npos) ? line.substr(0, spacePos) : line;
  std::transform(mnemonic.begin(), mnemonic.end(), mnemonic.begin(), ::tolower);

  std::string operandsStr =
      (spacePos != std::string::npos) ? line.substr(spacePos + 1) : "";
  ...
  if (mnemonic == "add" || mnemonic == "sub" || mnemonic == "and"
      || mnemonic == "or" || mnemonic == "slt") {
    return instruction.encodeRType();
  }

  if (mnemonic == "j" || mnemonic == "jal") {
    return instruction.encodeJType();
  }

  return instruction.encodeIType();
}
