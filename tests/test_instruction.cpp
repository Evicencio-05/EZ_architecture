...
TEST(InstructionTest, RTypeFormat) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::R_TYPE);
  EXPECT_EQ(instr.get_opcode(), 0);
  EXPECT_EQ(instr.get_rs(), 1);
  EXPECT_EQ(instr.get_rt(), 2);
  EXPECT_EQ(instr.get_rd(), 3);
  EXPECT_EQ(instr.get_shamt(), 0);
  EXPECT_EQ(instr.get_funct(), 0x20);
}
...
TEST(InstructionTest, RTypeWithShamt) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::R_TYPE);
  EXPECT_EQ(instr.get_rt(), 1);
  EXPECT_EQ(instr.get_rd(), 2);
  EXPECT_EQ(instr.get_shamt(), 4);
  EXPECT_EQ(instr.get_funct(), 0);
}
...
TEST(InstructionTest, RTypeSubtract) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::R_TYPE);
  EXPECT_EQ(instr.get_rs(), 3);
  EXPECT_EQ(instr.get_rt(), 4);
  EXPECT_EQ(instr.get_rd(), 5);
  EXPECT_EQ(instr.get_funct(), 0x22);
}
...
TEST(InstructionTest, RTypeAllZeros) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::R_TYPE);
  EXPECT_EQ(instr.get_rs(), 0);
  EXPECT_EQ(instr.get_rt(), 0);
  EXPECT_EQ(instr.get_rd(), 0);
  EXPECT_EQ(instr.get_shamt(), 0);
  EXPECT_EQ(instr.get_funct(), 0);
}
...
TEST(InstructionTest, ITypeFormat) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::I_TYPE);
  EXPECT_EQ(instr.get_opcode(), 8);
  EXPECT_EQ(instr.get_rs(), 1);
  EXPECT_EQ(instr.get_rt(), 2);
  EXPECT_EQ(instr.get_immediate(), 100);
}
...
TEST(InstructionTest, ITypeLoadWord) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::I_TYPE);
  EXPECT_EQ(instr.get_opcode(), 0x23);
  EXPECT_EQ(instr.get_rs(), 1);
  EXPECT_EQ(instr.get_rt(), 2);
  EXPECT_EQ(instr.get_immediate(), 8);
}
...
TEST(InstructionTest, ITypeStoreWord) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::I_TYPE);
  EXPECT_EQ(instr.get_opcode(), 0x2B);
  EXPECT_EQ(instr.get_rs(), 2);
  EXPECT_EQ(instr.get_rt(), 3);
  EXPECT_EQ(instr.get_immediate(), 12);
}
...
TEST(InstructionTest, ITypeNegativeImmediate) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::I_TYPE);
  EXPECT_EQ(instr.get_immediate(), -1);
}
...
TEST(InstructionTest, ITypeBranchEqual) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::I_TYPE);
  EXPECT_EQ(instr.get_opcode(), 4);
  EXPECT_EQ(instr.get_rs(), 1);
  EXPECT_EQ(instr.get_rt(), 2);
  EXPECT_EQ(instr.get_immediate(), 16);
}
...
TEST(InstructionTest, ITypeMaxImmediate) {
  ...
  EXPECT_EQ(instr.get_immediate(), 32767);
}
...
TEST(InstructionTest, ITypeMinImmediate) {
  ...
  EXPECT_EQ(instr.get_immediate(), -32768);
}
...
TEST(InstructionTest, JTypeFormat) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::J_TYPE);
  EXPECT_EQ(instr.get_opcode(), 2);
  EXPECT_EQ(instr.get_address(), 0x100000);
}
...
TEST(InstructionTest, JTypeJumpAndLink) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::J_TYPE);
  EXPECT_EQ(instr.get_opcode(), 3);
  EXPECT_EQ(instr.get_address(), 0x200000);
}
...
TEST(InstructionTest, JTypeMaxAddress) {
  ...
  EXPECT_EQ(instr.get_format(), InstructionFormat::J_TYPE);
  EXPECT_EQ(instr.get_address(), 0x3FFFFFF);
}
...
TEST(InstructionTest, RegisterFieldsMaxValues) {
  ...
  EXPECT_EQ(instr.get_rs(), 31);
  EXPECT_EQ(instr.get_rt(), 31);
  EXPECT_EQ(instr.get_rd(), 31);
}
...
TEST(InstructionTest, AlternatingBits) {
  ...
  EXPECT_EQ(instr.get_raw(), 0xAAAAAAAA);
  // Verify fields extract correctly even with alternating bits
  EXPECT_EQ(instr.get_opcode(), 42);  // 0b101010
}
